...
function FaqRow({ slug, index }: { slug: string; index: number }) {
  const { t } = useTranslation();
  const summaryRef = useRef<HTMLElement>(null);
  /**
   * Opening a row closes whichever row was open, and if that one sat higher up
   * the page everything below it jumps by the collapsed height — the answer
   * lands mid-scroll. Remember where the clicked question was before the
   * toggle, then scroll back by however far it moved.
   */
  const topBeforeToggle = useRef<number | null>(null);

  const rememberPosition = () => {
    topBeforeToggle.current = summaryRef.current?.getBoundingClientRect().top ?? null;
  };

  const keepQuestionInPlace = () => {
    const top = topBeforeToggle.current;
    topBeforeToggle.current = null;
    if (top === null || !summaryRef.current) return;

    const shift = summaryRef.current.getBoundingClientRect().top - top;
  ...
  return (
    ...
    >
      ...
      <details name={SECTION_IDS.faq} className="group" onToggle={keepQuestionInPlace}>
        <summary
          ref={summaryRef}
    ...
    >
      ...
      <ul className="border-t border-[var(--color-rule)]">
        {FAQ_SLUGS.map((slug, i) => (
          <FaqRow key={slug} slug={slug} index={i} />
        ))}
      </ul>
    </Section>
  );
